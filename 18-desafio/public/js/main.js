...
document.getElementById('btnProductoEnviar').addEventListener('click', (e) => {
    cleanErrors();
...
socket.on('new-product', productos => generarTablaProductos(productos));
socket.on('productos', productos => generarTablaProductos(productos.data));
...
socket.on('new-message', mensajes => generarTablaMensajes(mensajes));
socket.on('mensajes', mensajes => generarTablaMensajes(mensajes));
